package oddEvenLinkedList

import (
	"jiangbohhh/leetcode-go/structure/linkedlist"
	"reflect"
	"testing"
)

func Test_oddEvenList(t *testing.T) {
	type args struct {
		head []int
	}
	tests := []struct {
		name string
		args args
		want []int
	}{
		{
			name: "1->2->3->4->5 => 1->3->5->2->4",
			args: args{head: []int{1, 2, 3, 4, 5}},
			want: []int{1, 3, 5, 2, 4},
		},
		{
			name: "2->1->3->5->6->4->7 => 2->3->6->7->1->5->4",
			args: args{head: []int{2, 1, 3, 5, 6, 4, 7}},
			want: []int{2, 3, 6, 7, 1, 5, 4},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			head := linkedlist.ListToNode(tt.args.head)

			if got := linkedlist.NodeToList(oddEvenList(head)); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("oddEvenList() = %v, want %v", got, tt.want)
			}
		})
	}
}
